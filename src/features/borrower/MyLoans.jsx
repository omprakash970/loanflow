...
const myLoans = loans.filter((l) => l.borrowerId === "BRW-201" || l.borrowerId === "BRW-206");
...
const purposeIcon = {
  "Home Renovation":"🏠","Education":"🎓","Medical":"🏥","Vehicle Purchase":"🚗",
  "Business Expansion":"💼","Personal":"👤",
};

export default function MyLoans() {
  const totalOwed = myLoans.filter(l => l.status !== "Closed").reduce((a,l) => a + l.amount, 0);
      ...
      `}</style>
        ...
        <div className="ml-root">
          ...
          <div className="ml-summaries">
            {[
              { label:"Total Loans",    value:myLoans.length,               accent:"#818cf8" },
              { label:"Active",         value:active,                       accent:"#34d399" },
              { label:"Outstanding",    value:`$${totalOwed.toLocaleString()}`, accent:"#f59e0b" },
            ].map(s => (
          ...
          <div className="ml-cards">
            {myLoans.map(loan => {
              const s = statusCfg[loan.status] ?? statusCfg.Closed;
              return (
                  <div key={loan.id} className="loan-card" style={{ "--sc": s.color }}>
                    <div className="lc-icon">{purposeIcon[loan.purpose] ?? "◈"}</div>
                    <div className="lc-main">
                      <div className="lc-id">{loan.id}</div>
                      <div className="lc-purpose">{loan.purpose}</div>
                      <div className="lc-date">Started {loan.startDate}</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div className="lc-amount">${loan.amount.toLocaleString()}</div>
