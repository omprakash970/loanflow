...
import { borrowers } from "../../data/loans.mock";
...
export default function Borrowers() {
  ...
  const medCount = borrowers.filter(b=>b.riskLevel==="Medium").length;
      ...
      `}</style>
        ...
        <div className="bo-root">
          ...
          <div className="bo-risks">
            {[
              { level:"Low",    count:lowCount,  color:"#34d399", bg:"rgba(52,211,153,0.06)",  border:"rgba(52,211,153,0.2)",  glow:"rgba(52,211,153,0.12)",  bar:lowCount/borrowers.length*100  },
              { level:"Medium", count:medCount,  color:"#f59e0b", bg:"rgba(245,158,11,0.06)",  border:"rgba(245,158,11,0.2)",  glow:"rgba(245,158,11,0.12)",  bar:medCount/borrowers.length*100  },
              { level:"High",   count:highCount, color:"#f87171", bg:"rgba(248,113,113,0.06)", border:"rgba(248,113,113,0.2)", glow:"rgba(248,113,113,0.12)", bar:highCount/borrowers.length*100 },
            ].map(r => (
                <div
                    key={r.level}
                ...
                >
                  <div className="risk-card-label">{r.level} Risk</div>
          ...
          <div className="bo-toolbar">
            <input className="bo-search" placeholder="Search by name or ID…" value={search} onChange={e=>setSearch(e.target.value)} />
          ...
          <div className="table-wrap">
            <table className="bo-table">
              ...
              <tbody>
              {filtered.map(b => {
                const r = riskCfg[b.riskLevel] ?? riskCfg.Low;
                const initials = b.name.split(" ").map(n=>n[0]).join("");
                return (
                    <tr key={b.id}>
                      <td><span className="td-id">{b.id}</span></td>
                      <td>
                      ...
                      <td><span className="td-loans">{b.activeLoans}</span></td>
                      <td>
