...
const summaryStats = [
  { label: "Total Loans", value: loans.length, accent: "#2dd4bf" },
  { label: "Active",      value: loans.filter(l => l.status === "Active").length,  accent: "#34d399" },
  { label: "Overdue",     value: loans.filter(l => l.status === "Overdue").length, accent: "#f87171" },
  { label: "Total Volume",value: "$" + (loans.reduce((a,l) => a + l.amount, 0) / 1e6).toFixed(1) + "M", accent: "#818cf8" },
];
              ...
              {filtered.map(loan => {
                ...
                return (
                    <tr key={loan.id}>
                      ...
                      <td><span className="td-name">{loan.borrower}</span></td>
                      <td><span className="td-muted">{loan.lender}</span></td>
